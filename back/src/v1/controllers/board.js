...
exports.getOne = async (req, res) => {
  ...
  try {
    ...
    if (!board) return res.status(404).json("Board não encontrado");
...
exports.update = async (req, res) => {
  ...
  try {
    if (title === "") req.body.title = "Sem título";
    if (description === "") req.body.description = "Adicione uma descrição";
    const currentBoard = await Board.findById(boardId);
    if (!currentBoard) return res.status(404).json("Board não encontrado");
